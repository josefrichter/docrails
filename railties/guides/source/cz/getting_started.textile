  ...
  respond_to do |format|
    if @post.save
      format.html { redirect_to(@post,
                    :notice => 'Příspěvek byl úspěšně vytvořen.') }
      format.xml  { render :xml => @post,
...
Akce +create+ vytvoří nový objekt Post z dat, poskytnutých uživatelem přes formulář, které Rails spřístupní přes hash +params+. Po úspěšnem uložení nového příspěvku, vráti akce +create+ vhodný formát, který si užívatel vyžádal (v našem případě HTML). To pak přesměruje uživatele na výsledný příspěvek zobrazený akcí +show+ a nastaví upozornění pro uživatele, že příspěvek byl úspěšně vytvořen.

Jestli nebyl příspěvek úspěšně uložen kvůli validační chybě, pak vás kontroler pošle zpátky na akci +new+ s chybovými hláškami, takže uživatel má šanci opravit je a zkusit znovu.

Zpráva "Příspěvek byl úspěšně vytvořen." je uložena uvnitř Rails +flash+ hash-e, (většinou nazývaný jenom _flash_) takže zprávy se můžou přenést do jiné akce, poskytující uživatele s užitečnými informacemi o stavu jejich požadavku. V případě akce +create+ uživatel nikdy nevidí vyrenderovanou žádnou stránku během procesu vytváření příspěvku, protože je okamžitě přesměrován na nový příspěvek jakmile Rails uloží tento záznam. Flash se sebou nese zprávu do další akce, takže když je uživatel přesměrován zpátky na akci +show+, uvidí zprávu, která říka, že "Příspěvek byl úspěšně vytvořen.".
...
Akce +show+ používá +Post.find+ pro hledání jednoho záznamu v databáze podle hodnoty jeho id. Po jeho nalezení ho Rails zobrazí pomocí +show.html.erb+:
...
h4. Editace příspěvků

Stejně jako vytvoření nového příspěvku, tak i jeho editace je proces skládajíci se ze dvou částí. První krok je požadavek na +edit_post_path(@post)+ s částečným příspěvkem. Tohle zavolá akci +edit+ v kontroleri:
...
Po nalezení požadovaného příspěvku, Rails použije view +edit.html.erb+ pro jeho zobrazení:
...
Stejně jako s akcí +new+, akce +edit+ používá partial +form+, tentokrát ale, formulář provede akci PUT na PostsController a odesílací tlačítko zobrazí "Update Post"

Odeslání formuláře vytvořeného tímto view vyvolá akci +update+ na kontroleri:
...
V akci +update+, Rails nejprve použije +:id:+ parameter předaný z editačního view pro vyhledání záznamu v databáze, který se bude editovat. Volání +update_attributes+ pak převezme zbytek parametrů z požadavku a aplikuje je na tenhle záznam. Jestli všechno proběhne v pořádku, uživatel je přesměrován na +show+ view se zobrazeným příspěvkem. Jestli se vyskytnou ňejaké problémy, přesměruje se zpátky na view +edit+ pro jejich opravu.

h4. Mazání příspěvku

Nakonec, kliknutím jednoho z +destroy+ linků pošle přidružené id akci +destroy+:
...
Metoda +destroy+ na instanci modelu Active Record odstraní odpovídající záznam z databáze. Jakmile je to dokončeno, není záznam, který by se měl zobrazit, takže Rails přesměrují prohlížeč uživatele na index view pro tenhle model.
...
h4. Vykreslení partial formuláře
